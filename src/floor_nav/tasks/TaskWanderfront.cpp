...
TaskIndicator TaskWanderFront::iterate()
{
    ...
    // [0 : 30] degrees, etc
    try {
        // Use the value stored in sensor_map to implement the desired behaviors
        double min_forward = std::min(sensor_map[0],sensor_map[1]);
        // RCLCPP_INFO(getNode()->get_logger(), "min_forward: %f", min_forward);
        double linear = 0.0;
        ...
        // RCLCPP_INFO(getNode()->get_logger(), "linear: %f", linear);
        ...
        if (min_forward > cfg->safety_range) {
...
TaskIndicator TaskWanderFront::terminate()
{
    env->publishVelocity(0,0);
	return TaskStatus::TASK_TERMINATED;
}

DYNAMIC_TASK(TaskFactoryWanderFront);
