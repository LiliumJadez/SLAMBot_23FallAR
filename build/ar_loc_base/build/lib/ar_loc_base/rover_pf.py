...
class RoverPF(RoverOdo):
    ...
    def applyDisplacement(self, X, DeltaX, Uncertainty):
        ...
        rotated_delta_x = DeltaX[0] * cos_theta - DeltaX[1] * sin_theta
        rotated_delta_y = DeltaX[0] * sin_theta + DeltaX[1] * cos_theta

        X[0,0] += rotated_delta_x + numpy.random.normal(0, Uncertainty)
        X[1,0] += rotated_delta_y + numpy.random.normal(0, Uncertainty)
        X[2,0] += DeltaX[2] + numpy.random.normal(0, Uncertainty)
    ...
    def predict(self, logger, motor_state, drive_cfg, encoder_precision):
        ...
        logger.info("Shape of iW: "+str(iW.shape) + " Shape of S: "+str(S.shape) + " Shape of DeltaX: "+str(DeltaX.shape))
        for i in range(len(self.particles)):
            # logger.info("Particle shape: "+str(self.particles[i].shape))
            self.particles[i] = self.applyDisplacement(self.particles[i], DeltaX, encoder_precision)
            # logger.info("[AFTER] Particle shape: "+str(self.particles[i].shape))
    ...
    def evalParticleAR(self, X, Z, L, Uncertainty):
        ...
        z2 = self.getRotation(X[2,0]) @ L
        z2 = z2 + numpy.array([[Uncertainty],[Uncertainty]])
        # expected_angle = arctan2(dy, 2,0]
